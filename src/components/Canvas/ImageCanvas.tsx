...
import { Undo2, Redo2, Download, Scissors, ZoomIn, RotateCw, Eraser, Sparkles, Import, Upload, Loader2, Paintbrush } from 'lucide-react';
  ...
  return <Card className="h-full bg-card">
      ...
      <CardContent className="flex-1 flex flex-col">
        ...
        <div className="py-4 border-b border-border">
          ...
          {selectedTool === 'upscale' && <div className="mt-4 p-3 bg-muted rounded-lg">
        ...
        <div className="space-y-6 flex-1">
          {/* Editing Tools */}
          {showObjectEditTool && <EnhancedObjectEditTool imageUrl={asset.src} onEditComplete={handleObjectEdit} onCancel={() => setShowObjectEditTool(false)} className="w-full" />}
          
          {(showColorPanel || showStyleGallery) && <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              {showColorPanel && <ColorAdjustmentPanel onAdjustment={handleColorAdjustment} className="w-full" />}
              
              {showStyleGallery && <StyleFilterGallery onStyleApply={handleStyleFilter} className="w-full" />}
            </div>}
